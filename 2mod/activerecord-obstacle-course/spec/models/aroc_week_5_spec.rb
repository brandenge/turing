...
# ----------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    custom_results = User
                      .joins(:orders)
                      .select('users.name')
                      .select('COUNT(orders.id) AS total_order_count')
                      .group(:id)
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    # Why are COUNT(items.id) and COUNT(orders.id) the same?

    custom_results = User
                      .joins(:items)
                      .select('users.name')
                      .select('COUNT(items.id) AS total_item_count')
                      .group(:id)
                      .order(:name)

    # custom_results = User.joins(:items).select('users.name, COUNT(orders.id) AS total_item_count').group(:id).order(:name)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    data = User
            ...
            .select('TRUNC(orders.amount / COUNT(order_items.id)) AS avg_item_cost')
            .group(:name, :'orders.id')
            .order(user_name: :desc, avg_item_cost: :asc)


    # Why doesn't the below work, joining both :orders and :order_items?
  ...
  it '30. returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    # Why does the joins need the .select at the end?
    # And why does it work even without eager loading?

    orders = Order.joins(:items).select(:'items.name')\

    # Have to use .references with .includes
    # orders = Order.includes(:items).select(:name).references(:items)
    # .includes uses eager loading to avoid n+1 queries
