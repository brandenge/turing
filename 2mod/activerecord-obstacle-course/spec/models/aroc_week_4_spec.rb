...
# ----------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    # orders = @user_2.orders.filter do |order|
    #   order.items.include?(@item_4)
    # end

    # orders = @user_2.order_items.where(order_id: @item_4.orders.pluck(:id)).map do |order_item|
    #   order_item.order
    # end.uniq

    orders = @user_2.orders.where(id: @item_4.orders.pluck(:id))

    # orders = Order.joins(:order_items, :user).where("orders.user": @user_2, "order_items.item_id": @item_4.id)
    # -----------------------------------------------------------
