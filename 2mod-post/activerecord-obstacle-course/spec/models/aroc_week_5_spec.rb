...
# ----------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).select('users.name, COUNT(orders.id) AS total_order_count').group(:id).order(:total_order_count)
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:items).select('users.name, COUNT(items.id) AS total_item_count').group(:id).order(:name)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(:order_items).select('users.name AS user_name, orders.id AS order_id, TRUNC(orders.amount / COUNT(order_items.id)) AS avg_item_cost').group(:user_name, 'orders.id').order(name: :desc,  avg_item_cost: :asc)
    # ---------------------------------------------------------------
  ...
  it '30. returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    orders = Order.includes(:items).select(:name).references(:items) # Edit only this line
    # ------------------------------------------------------
